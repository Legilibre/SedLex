...
`--github-repository` and `--github-token` flags. Each article from the original bill will become a GitHub issue with:
...
Thus, each edit/commit will reference the corresponding article/issue (and vice versa) with clickable links when viewed on GitHub.
